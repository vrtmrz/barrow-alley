/**
 * A file entry disclosed only after sender acceptance.
 *
 * `id` is a session-scoped opaque identifier. It must never be a host path or
 * another stable source identifier. `hash` is declared metadata in Milestone 1;
 * calculating and verifying it against transferred bytes belongs to Milestone 2.
 */
export interface ManifestItem {
  /** Opaque identifier generated afresh for this sender session. */
  readonly id: string;
  /** Minimal safe name shown to the receiver; never an absolute or Vault path. */
  readonly displayName: string;
  /** Declared byte length. Milestone 2 verifies it against received bytes. */
  readonly size: number;
  /** Optional media type supplied by the source host. */
  readonly mimeType?: string;
  /** Lower-case SHA-256 hex declared by the sender and verified in Milestone 2. */
  readonly hash: string;
}
