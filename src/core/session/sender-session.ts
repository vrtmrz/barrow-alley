...
import { type SenderState, type SenderStateHandler, transitionSenderState } from "./state.js";
    ...
    #prepareManifest(sourceItems: readonly SourceItem[]): void {
        ...
        const manifestItems = sourceItems.map((sourceItem, index) => {
            if (sourceIds.has(sourceItem.id)) {
                throw new Error(`Duplicate source item ID: ${sourceItem.id}.`);
            }
            ...
            return sourceItem.mimeType === undefined
                ? base
                : { ...base, mimeType: sourceItem.mimeType };
        });
        ...
        if (validated.type !== "manifest") {
            throw new Error("Manifest validation returned another type.");
        }
