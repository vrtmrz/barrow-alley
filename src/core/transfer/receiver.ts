...
import type { FileBeginMessage, FileChunkMessage, FileEndMessage } from "../protocol/messages.js";
...
import { emitTransferProgress, type TransferProgressHandler } from "./progress.js";
    ...
    static async start(options: IncomingFileTransferOptions): Promise<IncomingFileTransfer> {
        ...
        if (begin.fileId !== expected.id || begin.displayName !== expected.displayName) {
            throw new TransferError(
                "TRANSFER_FAILED",
                "File-begin metadata does not match the manifest.",
            );
        }
        if (begin.size !== expected.size) {
            throw new TransferError(
                "SIZE_MISMATCH",
                "File-begin size does not match the manifest.",
            );
        }
        if (begin.hash !== expected.hash) {
            throw new TransferError(
                "HASH_MISMATCH",
                "File-begin hash does not match the manifest.",
            );
        }
    ...
    async write(message: FileChunkMessage): Promise<void> {
        ...
        if (message.index !== this.#nextIndex || message.offset !== this.#nextOffset) {
            return this.#reject(
                new TransferError(
                    "TRANSFER_FAILED",
                    "Chunk index or byte range is not contiguous.",
                ),
            );
        }
    ...
    async complete(message: FileEndMessage): Promise<void> {
        ...
        if (receivedHash !== this.#expected.hash) {
            return this.#reject(
                new TransferError(
                    "HASH_MISMATCH",
                    "Received bytes do not match the manifest hash.",
                ),
            );
        }
