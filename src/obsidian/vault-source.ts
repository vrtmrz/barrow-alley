 ...
 */
export class ObsidianVaultSource<TFile extends VaultBinaryFile = VaultBinaryFile>
    implements Source {
        ...
        for (const file of files) {
            if (paths.has(file.path)) {
                throw new VaultSourceError(
                    "DUPLICATE_FILE",
                    `The file '${file.name}' was selected twice.`,
                );
            }
            paths.add(file.path);
        }
    ...
    async list(): Promise<readonly SourceItem[]> {
        ...
        for (const selected of this.#selected) {
            ...
            prepared.push({
                ...
                sourceVersion: `${String(selected.file.stat.mtime)}:${
                    String(selected.file.stat.size)
                }`,
            });
        }
