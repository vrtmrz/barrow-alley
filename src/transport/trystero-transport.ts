...
export async function createTrysteroTransport(
    ...
    const relayTimeoutMs = options.relayConnectionTimeoutMs ?? DEFAULT_RELAY_CONNECTION_TIMEOUT_MS;
    const peerTimeoutMs = options.peerConnectionTimeoutMs ?? DEFAULT_PEER_CONNECTION_TIMEOUT_MS;
...
function encodeActionPayload(payload: unknown): EncodedActionPayload {
    if (
        !isRecord(payload) || payload.type !== "file-chunk" || !(payload.data instanceof Uint8Array)
    ) {
        return { data: payload };
    }
