...
export function findDependencyBoundaryViolations(root: string): BoundaryViolation[] {
    ...
    for (const sourceFile of sourceFiles) {
        const dependencies = extractSpecifiers(readFileSync(sourceFile, "utf8")).map((
            specifier,
        ) => ({
            specifier,
    ...
    for (const [importer, dependencies] of graph) {
        ...
        for (const dependency of dependencies) {
            ...
            const forbiddenLocalTarget = dependency.resolvedPath !== undefined &&
            ...
            if (forbiddenPackage || forbiddenLocalTarget) {
                violations.push({
                    ...
                    target: dependency.resolvedPath === undefined
                        ? undefined
